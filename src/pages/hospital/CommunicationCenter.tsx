...
import { Send, Bell, MessageSquare, Mail, CheckCircle, RefreshCw } from 'lucide-react';

export const CommunicationCenter: React.FC = () => {
  const { sendNotification, notifications, tokens } = useHospital();
  
  const [activeTab, setActiveTab] = useState<'push' | 'sms' | 'whatsapp' | 'email'>('push');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      sendNotification({
        ...
        message: activeTab === 'email' ? `${form.subject} - ${form.message}` : form.message
      });
      setSending(false);
      setSuccess(true);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Tabs */}
      <div className="flex bg-slate-100 rounded-2xl p-1 self-start max-w-md">
        {[
          { id: 'push', label: 'App Push', icon: <Bell size={13} /> },
          { id: 'sms', label: 'SMS broadcast', icon: <MessageSquare size={13} /> },
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-bold rounded-xl cursor-pointer border-none transition-all ${
        ...
        <div className="xl:col-span-2">
          <form onSubmit={handleSubmit} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-5">
            ...
            {form.recipientGroup === 'custom' && (
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1.5">Mobile Numbers / Emails (Comma separated)</label>
                <input
                  type="text"
                  required
                  value={form.customNumbers}
                  onChange={e => setForm(f => ({ ...f, customNumbers: e.target.value }))}
                  placeholder="e.g. +919876543210, user@email.com"
            ...
            {activeTab === 'email' && (
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1.5">Email Subject</label>
                <input
                  type="text"
                  required
                  value={form.subject}
                  onChange={e => setForm(f => ({ ...f, subject: e.target.value }))}
                  placeholder="e.g. Important Update: OPD Schedule Adjustments"
                  className="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs outline-none focus:border-blue-500"
                />
              </div>
            )}
            ...
            <div>
              ...
              <textarea
                ...
                rows={5}
                placeholder={activeTab === 'whatsapp' ? 'Enter WhatsApp template details...' : 'Type your system alert message here...'}
              ...
              <div className="flex justify-between items-center mt-1 text-[10px] text-slate-450">
                ...
                <span>Max 160 characters suggested for standard SMS</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-3 border-t border-slate-100">
              ...
              >
                ...
                {sending ? 'Broadcasting...' : `Send Broadcast via ${activeTab.toUpperCase()}`}
              </button>
              {success && (
